...
/**
 * Flags must not swallow the next flag as their value. Same bug class as the CLI's
 * --max-doc-freq, which parsed a missing value as 0 and turned a typo into a green verdict.
 */
...
const outPath = flag('out') ?? 'reports/cdx-check.json';
const limit = Number(flag('limit') ?? '50');
if (!Number.isInteger(limit) || limit <= 0) {
  process.stderr.write('\n  --limit must be a positive integer\n\n');
  process.exit(2);
}
const crawlsFlag = flag('crawls');
const targets = argv.filter((a, i) => !a.startsWith('--') && !argv[i - 1]?.startsWith('--'));

if (targets.length === 0) {
  process.stderr.write(
    '\n  usage: node scripts/cdx-check.ts <url-or-domain> [more...] [--crawls a,b] [--limit n] [--out path]\n\n',
  );
  process.exit(2);
}
