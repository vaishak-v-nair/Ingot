...
| 2026-07-30 | Gzip is a first-class input in the browser | The likeliest real corpus format was refused; now it streams through `DecompressionStream`, detected by magic bytes rather than filename. Two counters keep both truths: progress is compressed bytes against `file.size` (the only known total), while the hash digest and receipt count UNCOMPRESSED bytes — the CLI's documented rule — so a gzipped shard, its expanded twin, and a CLI scan of either all name the same corpus. Proven by unit test and by an in-browser end-to-end hash comparison. |
| 2026-07-30 | The browser-verification rig lives in the repo, and CI runs it | Probe scripts had accumulated in a session temp folder and a config directory — verification scattered across three spaces, none of them the product's. Consolidated into `e2e/`: the real shipped page driven in headless Chrome against synthetic deterministic indexes (no benchmark downloads), asserting the worker round-trip, every refusal state, cancel and double-drop, the receipt command, and gz/plain hash parity — 25 checks, `npm run e2e`, a CI job on every push. The scattered copies are deleted; the repo is the single space. |
